...
class LookRende extends Widget
{
  public $templates;
  public $messege;
  public function run()
  {
    $result = $this->parse(htmlspecialchars(json_decode($this->templates->template)));
    ...
    foreach ($param as $key => $item) {
      foreach ($item as $el => $val) {
        if (mb_stripos($result, $val) !== false) {
          $statys[$key][] = 1;
        } else {
          $statys[$key][] = 0;
        }
  ...
  public function parse($content)
  {
    ...
    foreach ($shortcodes as $shortcode) {
      ...
      if (in_array($shortcode, ['fields'])) {
        ...
        $result = preg_replace_callback("/$regexp/", array($this, 'parseSingle'), $result);
      }
      if (!empty($result)) {
        return $result;
      }else{
